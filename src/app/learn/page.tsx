...
function LearnContent() {
  ...
  const handleSearch = (e: FormEvent) => {
    ...
    setAnswers({});
    trackEvent('topic_searched', { topic: searchInput.trim() });
  ...
  // Track previous level so we can detect L1→L2 upgrade
  const prevLevelRef = useRef(level);
  useEffect(() => {
    if (prevLevelRef.current === 'L1' && level === 'L2') {
      trackEvent('level_upgraded', { topic, from: 'L1', to: 'L2' });
    }
  ...
  const handleQuizSubmit = () => {
    ...
    // score is computed inside submitQuiz; we optimistically log with answer count
    trackEvent('quiz_completed', { topic, level, answersGiven: answerArray.filter(Boolean).length });
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white p-6 font-sans flex flex-col items-center">
      ...
      <div className="w-full max-w-3xl mt-12 flex flex-col gap-8">
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <span style={{
                    ...
                    Generated by Gemini 2.0 Flash via Vertex AI
                  </span>
                </div>
                <motion.span
        ...
        <div style={{
          ...
          marginTop: '24px', flexWrap: 'wrap', paddingBottom: '40px',
        }}>
          {['Gemini 2.0 Flash', 'Vertex AI', 'Firebase', 'Cloud Run'].map(service => (
